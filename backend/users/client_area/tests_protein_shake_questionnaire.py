...
class ProteinShakeQuestionnaireNormalizationTests(SimpleTestCase):
    ...
    def test_same_every_day_post_workout_derives_from_training_meal(self):
        ...
        self.assertEqual(normalized["days"]["monday"]["selected_meal"], 3)
        self.assertEqual(normalized["selected_meals_by_day"]["monday"], 3)
...
class ProteinShakeFoodPreferenceTests(TestCase):
    ...
    def test_five_meals_with_shake_meal_3_persists_four_normal_combos_per_day(self):
        meal_counts = {day: 5 for day in WEEK_DAYS}
        shake_by_day = {day: 3 for day in WEEK_DAYS}
        protein_shake = {
            "enabled": True,
            ...
            "default_selected_meal": 3,
            "days": {day: {"enabled": True, "timing": "other", "selected_meal": 3} for day in WEEK_DAYS},
        }
        ...
        self.assertFalse(ClientMealComboSelection.objects.filter(user=user, meal_number=3).exists())
