...
def _normalize_height_cm(height_answer: dict[str, Any] | None):
...
def _calculate_bmr(gender: str | None, weight_kg: float, height_cm: float, age: int):
    ...
    # Preserve prototype behavior (female uses -160)
    if g == "male":
...
def _partition_day_multipliers_for_schedule(day_multipliers: list[float], workout_days: list[str]):
    ...
    # Keep descending allocation within each group so workout days receive the higher multipliers.
    assigned = {}
...
def build_questionnaire_results(context: BuildResultsContext):
    ...
    required_core = ["gender", "height", "weight", "date_of_birth", "goal", "lifestyle", "meal_plan_type", "workout_days", "meal_schedule", "training_schedule"]
