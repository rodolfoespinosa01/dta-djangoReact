...
def direct_client_tracking(request, user_id: int):
    ...
    profile = (
        ...
        .filter(user_id=user_id, sale_channel="dta_direct")
        .select_related("user")
        .first()
    )
    if not profile:
        return error(
            code="DIRECT_CLIENT_NOT_FOUND",
            message="Direct client not found.",
            http_status=404,
        )
