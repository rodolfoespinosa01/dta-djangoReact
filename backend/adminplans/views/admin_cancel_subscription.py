...
@api_view(['POST'])
@permission_classes([IsAuthenticated])  # Only logged-in users can cancel
def admin_cancel_subscription(request):
    ...
    # Get the admin's current profile
    profile = user.admin_profile
    subscription_id = profile.admin_stripe_subscription_id
    now = timezone.now()

    # Ensure the user has a valid subscription ID on file
    if not subscription_id:
        return Response({'error': 'No active Stripe subscription found'}, status=status.HTTP_400_BAD_REQUEST)

    try:
        # Set Stripe subscription to cancel at end of billing period
        stripe.Subscription.modify(
            subscription_id,
            cancel_at_period_end=True
        )

        # Retrieve full subscription to get current billing cycle end date
        subscription = stripe.Subscription.retrieve(subscription_id)
        current_period_end = subscription.get('current_period_end')

        # If Stripe provides the period end, convert it to Django timezone
        if current_period_end:
            subscription_end = timezone.datetime.fromtimestamp(current_period_end, tz=dt_timezone.utc)
        else:
            # Fallback if no period end is found (e.g. setup error or trial)
            subscription_end = profile.next_billing_date or (now + timezone.timedelta(days=14))

        # Update the AdminProfile with cancellation flags
        ...
        profile.next_billing_date = None  # Important to clear this to avoid confusion
        profile.save()

        # Create a record of the cancellation in the history model
        ...
        # Return confirmation and subscription end date
        return Response({
            'success': True,
